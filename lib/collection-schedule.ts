...
export const scheduleSchema = z.object({ enabled: z.boolean(), time: z.string().regex(/^([01]\d|2[0-3]):[0-5]\d$/), saturday: z.boolean(), sunday: z.boolean(), holidays: z.boolean(), instanceId: z.string().uuid().nullable() }).refine(config => !config.enabled || Boolean(config.instanceId), "Escolha o número remetente antes de ativar.");
export type CollectionScheduleConfig = z.infer<typeof scheduleSchema>;
export const DEFAULT_SCHEDULE: CollectionScheduleConfig = { enabled: false, time: '09:00', saturday: false, sunday: false, holidays: false, instanceId: null };
