...
export async function advanceBaseReport(report: CollectionReport) {
  const openFilter = report.mode === "ALL" ? "" : "&payload->>status=in.(PENDING,OVERDUE)";
  const rows = await supabaseRequest<{ external_id: string; payload: AsaasPayment; customer_payload: AsaasCustomer | null }[]>(`/rest/v1/asaas_base_payments?generation=eq.${report.source_generation}&select=external_id,payload,customer_payload&order=external_id.asc&limit=1000${openFilter}${report.source_cursor ? `&external_id=gt.${encodeURIComponent(report.source_cursor)}` : ""}`);
  const candidates = rows.filter(({ payload }) => {
    if (report.mode === "ALL") return true;
    if (!["PENDING", "OVERDUE"].includes(payload.status)) return false;
    return report.mode === "OPEN" || Boolean(collectionSchedule(payload.dueDate ?? null, report.report_date, rulesForCustomer(report.rule_config, payload.customer)).stage);
  });
  const excluded = await excludedAsaasCustomerIds(candidates.map(row => row.payload.customer));
  const snapshots = candidates.filter(row => !excluded.has(row.payload.customer)).map(({ payload: p, customer_payload: c }): ReportSnapshot => {
