...
export async function advanceBaseReport(report: CollectionReport) {
  ...
  const excluded = await excludedAsaasCustomerIds(rows.map(row => row.payload.customer));
  const snapshots = rows.filter(row => !excluded.has(row.payload.customer)).map(({ payload: p, customer_payload: c }): ReportSnapshot => {
