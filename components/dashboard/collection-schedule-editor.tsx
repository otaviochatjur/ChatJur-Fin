...
export function CollectionScheduleEditor() {
  ...
  useEffect(() => {
    let stopped = false;
    Promise.all([fetch('/api/collections/schedule'), fetch('/api/collections/instances')]).then(async ([scheduleResponse, instancesResponse]) => {
      const schedule = await scheduleResponse.json(), numbers = await instancesResponse.json();
      if (!scheduleResponse.ok) throw new Error(schedule.error); if (!instancesResponse.ok) throw new Error(numbers.error);
      if (!stopped) { setConfig(schedule.config); setJobs(schedule.jobs); setInstances(numbers.instances ?? []); setLoaded(true); }
  ...
  return <section className="space-y-5 rounded-2xl bg-card p-5">
    ...
    <fieldset disabled={!loaded || busy} className="space-y-4">
      ...
      <label className="block max-w-md text-sm">Número remetente<select className="mt-1 block h-10 w-full rounded-md border bg-card px-3" value={config.instanceId ?? ''} onChange={event => setConfig({ ...config, instanceId: event.target.value || null })}><option value="">Escolha um número conectado</option>{instances.map(instance => <option key={instance.id} value={instance.id}>{instance.name || 'WhatsApp'} · {instance.display_phone_number || instance.phone_id || instance.id} · {instance.approved_template_count ?? 0} templates</option>)}</select></label>
    ...
    <div className="space-y-2"><h3 className="text-sm font-medium">Últimas execuções</h3>{!jobs.length && <p className="text-sm text-muted-foreground">Nenhuma execução programada registrada.</p>}{jobs.map(job => <div key={job.run_date} className="rounded-lg bg-muted/30 p-3 text-sm"><p>{job.run_date.split('-').reverse().join('/')} · {job.status === 'COMPLETE' ? 'Concluída' : job.status === 'RUNNING' ? 'Em andamento' : 'Requer revisão'} · {job.sent} enviados · {job.skipped} ignorados</p>{job.instance_id && <p className="text-xs text-muted-foreground">Remetente: {instances.find(instance => instance.id === job.instance_id)?.display_phone_number ?? job.instance_id}</p>}{job.error && <p className="mt-1 text-destructive">{job.error} Use o envio manual após conferir o histórico; esta execução não será repetida automaticamente.</p>}</div>)}</div>
  </section>;
}
