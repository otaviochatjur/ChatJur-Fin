    ...
    <div className="space-y-5">
      <section className="rounded-2xl border border-slate-200 dark:border-border bg-white dark:bg-card shadow-[0_10px_30px_rgba(15,23,42,0.04)]">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 dark:border-border p-5">
          <div><h2 className="font-semibold">{title}</h2><p className="mt-1 text-sm text-slate-500 dark:text-muted-foreground">{subtitle}</p></div>
        ...
        <Table className="table-fixed">
          <TableHeader>
            <TableRow>
              <ResizableTh {...sorting.header("primary")} width={widths.getWidth("primary", 220)} onResizeStart={widths.startResize("primary", 220)}>{columnLabel}</ResizableTh>
              {columns.isVisible("clients") && <ResizableTh {...sorting.header("clients")} width={widths.getWidth("clients")} onResizeStart={widths.startResize("clients")} className="text-right">Clientes</ResizableTh>}
              ...
              {columns.isVisible("links") && <ResizableTh {...sorting.header("links")} width={widths.getWidth("links")} onResizeStart={widths.startResize("links")} className="text-right">Links</ResizableTh>}
          ...
          <TableBody>
            {filtered.length === 0 && <TableRow><TableCell colSpan={columns.visibleCount + 2} className="text-center text-sm text-slate-500 dark:text-muted-foreground">{scoped.length === 0 ? emptyMessage : "Nenhum cadastro nesse filtro."}</TableCell></TableRow>}
            {sorting.rows(filtered, actor => ({ primary: actor.name, clients: metrics[actor.id]?.clients ?? 0, mrr: metrics[actor.id]?.mrr ?? 0, links: metrics[actor.id]?.links ?? 0, status: actor.status === "ACTIVE" ? "Ativo" : "Inativo" })).map((actor) => {
