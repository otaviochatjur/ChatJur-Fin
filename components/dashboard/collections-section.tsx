...
export function CollectionsSection({ payments }: { payments: Payment[]; customers: Customer[] }) {
  ...
  useEffect(() => {
    ...
    fetch("/api/collections/instances", { signal: AbortSignal.timeout(20000) }).then(async response => {
      ...
      if (!stopped) { setInstances(data.instances ?? []); setSenderId(current => current || data.instances?.find((instance: ChatInstance) => (instance.approved_template_count ?? 0) > 0)?.id || data.instances?.[0]?.id || ""); }
  ...
  return <div className="space-y-6">
    ...
    <fieldset disabled={busy} hidden={view!=="reports"}><CollectionReports approvedTemplates={approvedTemplates} selectedPayments={selected} onSelectionChange={setSelected} onSelect={(selected, _saved, data) => { setReport(selected); setReportData(data); setRows(null); setResults({}); setSelected(new Set()); setProgress(null); }} /></fieldset>
    ...
    <section className="rounded-2xl border bg-card p-6">
      ...
      <div className="mt-5 flex flex-wrap items-end gap-3"><label className="min-w-64 text-sm">Enviar pelo número<select aria-label="Número remetente do Chat Jurídico" className="mt-1 block h-10 w-full rounded-md border bg-card px-3" value={senderId} disabled={busy} onChange={event => { setSenderId(event.target.value); setRows(null); setResults({}); setSelected(new Set()); }}><option value="">Escolha um número conectado</option>{instances.map(instance => <option key={instance.id} value={instance.id}>{instance.name || "WhatsApp"} · {instance.display_phone_number || instance.phone_id || instance.id} · {instance.approved_template_count ?? 0} templates</option>)}</select></label><Button disabled={busy || !senderId || !report || report.status !== "COMPLETE" || report.mode === "ALL" || !selected.size} onClick={load}>{busy ? "Processando…" : `Preparar mensagens selecionadas (${selected.size})`}</Button><Button disabled={busy} variant="outline" onClick={exportHistory}>Exportar histórico do mês</Button></div>
      {instanceError && <p role="alert" className="mt-3 text-sm text-destructive">{instanceError}</p>}
