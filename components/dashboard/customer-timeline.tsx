...
export function CustomerTimeline({ customerId, subscriptions, events, onChanged }: { subscriptions: Subscription[]; customerId: string; events: CustomerActivity[]; onChanged: () => void }) {
  ...
  return <section className="space-y-4 rounded-xl border border-border/50 bg-muted/40 p-4">
    <div className="flex items-center justify-between gap-2"><h3 className="font-semibold">Acompanhamento do cliente</h3>{editingId && <span className="inline-flex items-center gap-1 rounded-full bg-amber-100 dark:bg-amber-950 px-2.5 py-1 text-xs font-medium text-amber-800 dark:text-amber-300">Editando registro<button type="button" onClick={resetForm} className="ml-1 rounded-full hover:bg-amber-200"><X className="size-3" /></button></span>}</div>
    <form onSubmit={save} className="space-y-3">
      <label className="block text-sm">Tipo de registro<select className="mt-1 h-10 w-full rounded-md border bg-white dark:bg-card px-3" value={type} onChange={event => { setType(event.target.value as CustomerActivity["type"]); setSubscriptionId(""); }}>{Object.entries(activityLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>
      ...
      {needsSubscription && <label className="block text-sm">Assinatura afetada<select required className="mt-1 h-10 w-full rounded-md border bg-white dark:bg-card px-3" value={subscriptionId} onChange={event => { setSubscriptionId(event.target.value); const chosen = subscriptions.find(s => s.id === event.target.value); if (chosen && changingPlan) { setNextPeriod(chosen.billing_period); setNextValue(String(chosen.value)); setNextName(chosen.plan_name_raw ?? "Plano"); } }}><option value="">Selecione a assinatura</option>{subscriptions.filter(s => s.status === "ACTIVE" || s.id === subscriptionId).map(s => <option key={s.id} value={s.id}>{s.display_plan_name ?? "—"}{s.display_actor_label ? ` · ${s.display_actor_label}` : ""} · {s.billing_period === "ANNUAL" ? "Anual" : "Mensal"} · {money.format(s.value)}</option>)}</select></label>}
      {needsSubscription && !subscriptions.some(s => s.status === "ACTIVE") && <p className="text-sm text-muted-foreground">Nenhuma assinatura ativa disponível. Confira a assinatura existente na seção abaixo e confirme seu status antes de registrar uma alteração.</p>}
      {changingPlan && <fieldset className="space-y-3 rounded-lg border bg-white dark:bg-card p-4">
        <legend className="px-1 font-semibold">Mudança de plano / periodicidade</legend>
        {subscription && <p className="text-sm text-slate-500 dark:text-muted-foreground">Antes: {subscription.display_plan_name ?? "—"} · {subscription.billing_period === "ANNUAL" ? "Anual" : "Mensal"} · {money.format(subscription.value)}</p>}
        ...
        <p className="text-sm text-slate-500 dark:text-muted-foreground">O novo valor passa a valer na data de vigência nos relatórios internos. Sem cobrança proporcional ou créditos. Este registro não altera cobranças no Asaas.</p>
      </fieldset>}
      ...
      {type === "REACTIVATION" && <p className="text-sm text-slate-500 dark:text-muted-foreground">Ao salvar, o cadastro do cliente volta automaticamente para o status Ativo. Lembre-se de garantir que exista uma assinatura ativa: pague por um novo link (a sincronização cria a assinatura sozinha) ou cadastre o plano manualmente em Assinaturas.</p>}
      {itemized && <fieldset className="space-y-3 rounded-lg border bg-white dark:bg-card p-4">
        <legend className="px-1 text-sm font-semibold">{type === "UPSELL" ? "Itens adicionados" : "Itens removidos"}</legend>
        <p className="text-sm text-slate-500 dark:text-muted-foreground">Selecione os itens e informe o valor mensal total de cada um para a quantidade escolhida.</p>
        {(Object.entries(itemCatalog) as [ActivityItem["kind"], { label: string; quantities: number[] }][]).map(([kind, config]) => {
          ...
          return <div key={kind} className="rounded-lg border p-3">
            ...
            {item && <div className="mt-3 grid gap-3 sm:grid-cols-2">
              {config.quantities.length > 0 && <label className="text-sm">Quantidade<select className="mt-1 h-9 w-full rounded-md border bg-white dark:bg-card px-3" value={item.quantity ?? ""} onChange={event => updateItem(kind, { quantity: Number(event.target.value) })}>{config.quantities.map(quantity => <option key={quantity} value={quantity}>{quantity}</option>)}</select></label>}
        ...
        <p aria-live="polite" className="text-lg font-semibold text-blue-800 dark:text-blue-300">{type === "UPSELL" ? "Acréscimo total" : "Redução total"}: {money.format(total)}</p>
      ...
      {type === "CANCELLATION" && <p className="text-sm text-slate-500 dark:text-muted-foreground">Ao salvar, o cadastro do cliente volta automaticamente para o status Cancelado.</p>}
      ...
      <p className="text-xs text-slate-500 dark:text-muted-foreground">Este registro alimenta as métricas comerciais. {["CANCELLATION", "REACTIVATION"].includes(type) ? "O status do cliente é atualizado automaticamente." : "Atualize o cadastro abaixo quando houver mudança de status."} Assinaturas e recebimentos continuam vinculados aos pagamentos.</p>
    ...
    <div className="space-y-3 border-t pt-4">
      ...
      {events.length === 0 && <p className="text-sm text-slate-500 dark:text-muted-foreground">Nenhum acompanhamento registrado.</p>}
      <div className="max-h-96 space-y-3 overflow-y-auto">{[...events].sort((a, b) => b.occurredOn.localeCompare(a.occurredOn) || b.createdAt.localeCompare(a.createdAt)).map(event => <article key={event.id} className={"rounded-lg border bg-white dark:bg-card p-3 text-sm" + (editingId === event.id ? " border-amber-300 ring-1 ring-amber-200" : "")}>
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button type="button" variant="ghost" size="icon" className="size-7 text-red-600 dark:text-red-300 hover:text-red-700 dark:hover:text-red-300" disabled={deletingId === event.id} onClick={() => remove(event.id)} title="Excluir registro"><Trash2 className="size-3.5" /><span className="sr-only">Excluir</span></Button>
          </div>
        </div>
        {event.amount > 0 && <p className="mt-1 font-medium text-blue-700 dark:text-blue-300">{money.format(event.amount)}</p>}
        ...
        {event.subscriptionId && (event.type === "UPSELL" || event.type === "DOWNSELL") && <p className="mt-1 text-slate-500 dark:text-muted-foreground">Assinatura: {subscriptions.find(s => s.id === event.subscriptionId)?.display_plan_name ?? "assinatura removida"} · Impacto no MRR: {money.format(itemizedMrrDelta(event))} · Vigência: {event.occurredOn.split("-").reverse().join("/")}</p>}
        {event.notes && <p className="mt-1 whitespace-pre-wrap break-words text-slate-600 dark:text-muted-foreground">{event.notes}</p>}
      </article>)}</div>
    </div>
  </section>;
}
