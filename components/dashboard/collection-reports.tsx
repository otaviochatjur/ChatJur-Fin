...
export function CollectionReports({ onSelect, onCustomerClick, selectedPayments, onSelectionChange, approvedTemplates }: { approvedTemplates: string[]; selectedPayments: Set<string>; onSelectionChange: (ids: Set<string>) => void; onSelect: (report: CollectionReport, savedPreviews: CollectionPreview[], rows: ReportRow[]) => void; onCustomerClick: (customerId: string) => void }) {
  ...
  const ordered = sorting.rows(filtered, row => ({ client: row.snapshot.name, due: row.snapshot.due_date, value: Number(row.snapshot.value), status: row.snapshot.status, method: collectionPaymentMethod(row.snapshot.billing_type), stage: [row.snapshot.stage ?? 'Sem envio hoje', ...row.snapshot.warnings].join(' · '), sent: row.sent_today_at ?? '' }));
  ...
  const headerSelectable = visibleSelectable.filter(row => !row.sent_today_at);
  const allSelected = headerSelectable.length > 0 && headerSelectable.every(row => selectedPayments.has(row.snapshot.payment_id));
  ...
  return <section className="space-y-5 rounded-2xl border bg-card p-5 sm:p-6">
    ...
    {selected && <>
      ...
      <p className="text-sm text-muted-foreground">{selectedPayments.size} cobrança(s) apta(s) selecionada(s). A seleção do cabeçalho ignora quem já recebeu mensagem hoje. Para repetir um envio, marque a cobrança individualmente; o sistema pedirá confirmação antes do disparo.</p>
      <Table><TableHeader><TableRow><TableHead><input type="checkbox" aria-label="Selecionar cobranças aptas ainda não enviadas hoje em todas as páginas" disabled={!headerSelectable.length} checked={allSelected} ref={element => { if (element) element.indeterminate = !allSelected && headerSelectable.some(row => selectedPayments.has(row.snapshot.payment_id)); }} onChange={event => { const next = new Set(selectedPayments); for (const row of headerSelectable) { if (event.target.checked) next.add(row.snapshot.payment_id); else next.delete(row.snapshot.payment_id); } onSelectionChange(next); }} /></TableHead>{[['client','Cliente',220],['due','Vencimento',140],['value','Valor',120],['method','Forma de pagamento',180],['status','Status',140],['sent','Envio hoje',180],['stage','Etapa / pendências',240]].map(([key,label,width]) => <ResizableTh key={key} {...sorting.header(String(key))} onSort={() => { sorting.header(String(key)).onSort(); setPage(0); }} width={widths.getWidth(String(key),Number(width))} onResizeStart={widths.startResize(String(key),Number(width))}>{label}</ResizableTh>)}</TableRow></TableHeader><TableBody>{ordered.slice(page*50,page*50+50).map(row => { const blocked = selectionBlock(row); return <TableRow key={row.id}><TableCell><input type="checkbox" title={blocked ?? (row.sent_today_at ? 'Já recebeu mensagem hoje; selecione apenas se quiser repetir' : 'Apta para preparar e enviar')} aria-label={`Selecionar cobrança de ${row.snapshot.name}, vencimento ${row.snapshot.due_date}`} disabled={Boolean(blocked)} checked={selectedPayments.has(row.snapshot.payment_id)} onChange={event => toggle(row.snapshot.payment_id,event.target.checked)} /></TableCell><TableCell><button className="text-left font-medium underline-offset-4 hover:underline" aria-haspopup="dialog" onClick={() => row.internal_customer_id ? onCustomerClick(row.internal_customer_id) : setError("Este cliente ainda não está vinculado à base interna.")}>{row.snapshot.name}</button><p className="text-xs text-muted-foreground">{row.snapshot.email}</p></TableCell><TableCell>{row.snapshot.due_date?.split("-").reverse().join("/") ?? "—"}</TableCell><TableCell>{money.format(row.snapshot.value)}</TableCell><TableCell>{collectionPaymentMethod(row.snapshot.billing_type)}</TableCell><TableCell>{row.snapshot.status}</TableCell><TableCell>{row.sent_today_at ? <span className="inline-flex rounded-full bg-emerald-100 px-2 py-1 text-xs font-medium text-emerald-800 dark:bg-emerald-950 dark:text-emerald-200">Enviada às {new Date(row.sent_today_at).toLocaleTimeString("pt-BR", { timeZone: "America/Sao_Paulo", hour: "2-digit", minute: "2-digit" })}</span> : <span className="text-xs text-muted-foreground">Não enviada</span>}</TableCell><TableCell>{row.snapshot.stage ?? "Sem envio hoje"}{blocked && <p className="text-xs text-amber-700 dark:text-amber-300">{blocked}</p>}{!!row.snapshot.warnings.length && <p className="text-xs text-amber-700 dark:text-amber-300">{row.snapshot.warnings.join(" · ")}</p>}</TableCell></TableRow>})}{!filtered.length && <TableRow><TableCell colSpan={8} className="py-6 text-center text-muted-foreground">Nenhum registro neste relatório ou filtro.</TableCell></TableRow>}</TableBody></Table>
