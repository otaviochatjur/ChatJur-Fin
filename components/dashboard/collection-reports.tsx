...
export function CollectionReports({ onSelect, selectedPayments, onSelectionChange }: { selectedPayments: Set<string>; onSelectionChange: (ids: Set<string>) => void; onSelect: (report: CollectionReport, savedPreviews: CollectionPreview[], rows: ReportRow[]) => void }) {
  ...
  const canSelect = (row: ReportRow) => selected?.mode !== 'ALL' && ['PENDING','OVERDUE'].includes(row.snapshot.status);
