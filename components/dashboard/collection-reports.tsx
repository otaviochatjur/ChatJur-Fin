...
export function CollectionReports({ onSelect }: { onSelect: (report: CollectionReport, savedPreviews: CollectionPreview[], rows: ReportRow[]) => void }) {
  ...
  async function exportReport() {
    ...
    const data = rows.map(({ snapshot: r }) => ({ "Pagamento Asaas": r.payment_id, "Cliente Asaas": r.customer_id, "Nome": r.name, "E-mail": r.email, "Celular": r.phone, "Vencimento": r.due_date, "Valor": r.value, "Status Asaas": r.status, "Forma de pagamento": r.billing_type, "Descrição": r.description, "Dias": r.days, "Etapa": r.stage ?? "Sem envio hoje", "Gatilho": r.trigger, "Data nominal": r.nominal, "Data efetiva": r.effective, "Fatura": r.invoice_url, "Boleto": r.bankslip_url, "Pix copia e cola": r.pix_payload, "Pix expiração": r.pix_expiration, "Pendências": r.warnings.join("; ") }));
  ...
  const filtered = rows.filter(row => [row.snapshot.name,row.snapshot.email,row.snapshot.payment_id,row.snapshot.stage,row.snapshot.status].some(value => value?.toLowerCase().includes(search.toLowerCase())));
  return <section className="space-y-5 rounded-2xl border bg-card p-5 sm:p-6">
    ...
    <p className="text-xs text-muted-foreground">Régua do dia: somente etapas previstas. Vencidas e a vencer: PENDING e OVERDUE. Todas: histórico completo, sem consultar Pix. Etapas em dias não úteis são transferidas para o próximo dia útil; D+30 gera uma mensagem, sem cancelamento automático.</p>
    ...
    {selected && <>
      ...
      <Table><TableHeader><TableRow><TableHead>Cliente</TableHead><TableHead>Vencimento</TableHead><TableHead>Valor</TableHead><TableHead>Status</TableHead><TableHead>Etapa / pendências</TableHead></TableRow></TableHeader><TableBody>{filtered.slice(page*50,page*50+50).map(row => <TableRow key={row.id}><TableCell><button className="text-left font-medium underline-offset-4 hover:underline" onClick={() => setDetail(row)}>{row.snapshot.name}</button><p className="text-xs text-muted-foreground">{row.snapshot.email}</p></TableCell><TableCell>{row.snapshot.due_date?.split("-").reverse().join("/") ?? "—"}</TableCell><TableCell>{money.format(row.snapshot.value)}</TableCell><TableCell>{row.snapshot.status}</TableCell><TableCell>{row.snapshot.stage ?? "Sem envio hoje"}{!!row.snapshot.warnings.length && <p className="text-xs text-amber-700 dark:text-amber-300">{row.snapshot.warnings.join(" · ")}</p>}</TableCell></TableRow>)}{!filtered.length && <TableRow><TableCell colSpan={5} className="py-6 text-center text-muted-foreground">Nenhum registro neste relatório ou filtro.</TableCell></TableRow>}</TableBody></Table>
