...
export async function GET() {
  ...
  try {
    const [config, jobs] = await Promise.all([readSchedule(), supabaseRequest<ScheduleJob[]>('/rest/v1/collection_schedule_jobs?select=run_date,status,cursor,sent,skipped,error,updated_at&order=run_date.desc&limit=15')]);
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!parsed.success) return Response.json({ error: 'Informe um horário válido e as opções de envio.' }, { status: 400 });
