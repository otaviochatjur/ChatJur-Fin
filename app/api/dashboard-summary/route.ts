...
import { computeActorMetrics, sumImplementationRevenue, sumRealizedRevenue, type CommercialActor, type Customer, type ImplementationPayment, type Payment, type PaymentLink, type Subscription } from "@/lib/metrics";
import { supabaseRequest } from "@/lib/supabase-server";

export async function GET() {
  try {
    const [actors, links, baseSubscriptions, payments, implementationPayments, customers] = await Promise.all([
      ...
      supabaseRequest<Pick<Customer, "id" | "acquisition_actor_id">[]>("/rest/v1/customers?select=id,acquisition_actor_id"),
    ]);

    const subscriptions = effectiveSubscriptions(baseSubscriptions, await readCustomerActivities());
    const metrics = computeActorMetrics(actors, subscriptions, links, customers);
