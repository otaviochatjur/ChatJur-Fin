...
import { getIntegration, sealSecret, secretHash, validateAsaasKey } from "@/lib/integrations-server";
...
export async function GET() {
  try {
    ...
    return Response.json({ asaas: { configured: Boolean(asaas || (tenant.legacy && process.env.ASAAS_API_KEY)), environment: asaas?.environment ?? (process.env.ASAAS_BASE_URL?.includes("sandbox") ? "sandbox" : "production") }, chat: { configured: Boolean(chat) }, tally: { configured: Boolean(tally), formId: tally?.account_id ?? "2EWBOV", webhookPath: `/api/webhooks/tally?account=${tenant.id}` } }, { headers: { "Cache-Control": "no-store" } });
...
export async function POST(request: Request) {
  ...
  if (!body || !["asaas", "chat-juridico", "tally"].includes(body.provider) || typeof body.apiKey !== "string" || body.apiKey.trim().length < 10 || body.apiKey.length > 8192 || !["sandbox", "production"].includes(body.environment)) return Response.json({ error: "Informe uma chave válida e o ambiente." }, { status: 400 });
  try {
    const tenant = await currentTenant();
